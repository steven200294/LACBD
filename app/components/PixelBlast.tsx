...
const createLiquidEffect = (texture: THREE.Texture, opts?: { strength?: number; freq?: number }) => {
  ...
  return new Effect('LiquidEffect', fragment, {
    // @ts-expect-error - mixed uniform types
    uniforms: new Map([
...
const PixelBlast = ({
  ...
  useEffect(() => {
    ...
    if (!mustReinit && prevConfigRef.current) {
      for (const k of needsReinitKeys) if (prevConfigRef.current[k] !== (cfg as Record<string, unknown>)[k]) { mustReinit = true; break; }
    }
    if (mustReinit) {
      if (threeRef.current) {
        ...
        (t.quad as THREE.Mesh)?.geometry.dispose();
      ...
      const setSize = () => {
        ...
        if ((threeRef.current?.composer as EffectComposer))
          (threeRef.current?.composer as EffectComposer).setSize(renderer.domElement.width, renderer.domElement.height);
      ...
      if (noiseAmount > 0) {
        ...
        const noiseEffect = new Effect('NoiseEffect',
          `uniform float uTime; uniform float uAmount; float hash(vec2 p){return fract(sin(dot(p,vec2(127.1,311.7)))*43758.5453);} void mainUv(inout vec2 uv){} void mainImage(const in vec4 inputColor,const in vec2 uv,out vec4 outputColor){float n=hash(floor(uv*vec2(1920,1080))+floor(uTime*60.0));float g=(n-0.5)*uAmount;outputColor=inputColor+vec4(vec3(g),0.0);}`,
      ...
      const t = threeRef.current!;
      const u = t.uniforms as typeof threeRef.current extends null ? never : Record<string, { value: unknown }>;
      (u.uShapeType as { value: number }).value = SHAPE_MAP[variant] ?? 0;
      (u.uPixelSize as { value: number }).value = pixelSize * (t.renderer as THREE.WebGLRenderer).getPixelRatio();
      (u.uColor as { value: THREE.Color }).value.set(color);
      (u.uScale as { value: number }).value = patternScale;
      (u.uDensity as { value: number }).value = patternDensity;
      (u.uPixelJitter as { value: number }).value = pixelSizeJitter;
      (u.uEnableRipples as { value: number }).value = enableRipples ? 1 : 0;
      (u.uRippleIntensity as { value: number }).value = rippleIntensityScale;
      (u.uRippleThickness as { value: number }).value = rippleThickness;
      (u.uRippleSpeed as { value: number }).value = rippleSpeed;
      (u.uEdgeFade as { value: number }).value = edgeFade;
      if (transparent) (t.renderer as THREE.WebGLRenderer).setClearAlpha(0);
